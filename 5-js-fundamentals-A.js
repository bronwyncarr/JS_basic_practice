...
const arrayMultiply = (num1, num2, cb) => {
  const sum = num1 * num2;
  return cb(sum);
};

const result = arrayMultiply(1, 2, (sum) => {
  return arr.map((number) => number * sum);
});
...
const arrayMultiplyAgain = (num, arr) => arr.map((element) => element * num);

const moreArrayMultiply = (num, arr, cb) => {
  return cb(num, arr);
};

const resultTwo = moreArrayMultiply(2, [1, 2, 3], arrayMultiplyAgain);
